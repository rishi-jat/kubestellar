...
# Wait for job completion with robust handling of mixed pod statuses
echo "Waiting for its-with-clusteradm job to complete..."
kubectl wait -n its1-system job.batch/its-with-clusteradm --for condition=Complete --timeout 400s || {
    echo "Job wait timed out. Checking job and pod status..."
    echo "Job status:"
    kubectl -n its1-system get job its-with-clusteradm -o yaml
    echo "Pod status:"
    kubectl -n its1-system get pods -l job-name=its-with-clusteradm -o yaml
    echo "Pod logs:"
    kubectl -n its1-system get pods -l job-name=its-with-clusteradm -o name | while read pod; do
        echo "=== Logs for $pod ==="
        kubectl -n its1-system logs $pod || true
    done
...
# Wait for job completion with robust handling of mixed pod statuses
echo "Waiting for its-with-clusteradm job to complete (second check)..."
kubectl --context $HOSTING_CONTEXT wait -n its1-system job.batch/its-with-clusteradm --for condition=Complete --timeout 400s || {
    echo "Job wait timed out. Checking job and pod status..."
    echo "Job status:"
    kubectl --context $HOSTING_CONTEXT -n its1-system get job its-with-clusteradm -o yaml
    echo "Pod status:"
    kubectl --context $HOSTING_CONTEXT -n its1-system get pods -l job-name=its-with-clusteradm -o yaml
    echo "Pod logs:"
    kubectl --context $HOSTING_CONTEXT -n its1-system get pods -l job-name=its-with-clusteradm -o name | while read pod; do
        echo "=== Logs for $pod ==="
        kubectl --context $HOSTING_CONTEXT -n its1-system logs $pod || true
    done
    # Check if at least one pod succeeded (which should be sufficient for the job to be considered complete)
